<?php
    ...
    try {
		...
		$query =
			"UPDATE [auxdw].[comercio.tbl_tianguis]
				SET nombre = :nombre,
					dia = :dia,
					modified = 'M'
		        WHERE id = :id";
		...
		for($i = 0; $i < count($tableData); $i++) {
			...
			$stmt->bindParam(':dia', $tableData[$i]['dia'], PDO::PARAM_STR);
		...
		$query =
			"DELETE FROM [auxdw].[comercio.tbl_tianguis]
				WHERE id = :id";
		$stmt = $conn->prepare($query);
		...
		$query =
			"INSERT INTO [dbo].[tbl_log_updates](username, changed_table)
				VALUES (:username, '[auxdw].[comercio.tbl_tianguis]')";
		$stmt = $conn->prepare($query);
