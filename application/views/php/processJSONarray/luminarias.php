<?php
    ...
    try {
		...
		$query =
			"UPDATE [auxdw].[servicios_publicos.tbl_luminarias]
				SET f_secundaria = :f_secundaria,
					modified = 'M',
            		id_material =
		...
		for($i = 0; $i < count($tableData); $i++) {
			$stmt->bindParam(':f_secundaria', $tableData[$i]['f_secundaria'], PDO::PARAM_STR);
		...
		$query =
			"UPDATE [auxdw].[servicios_publicos.tbl_luminarias]
				SET id_material = 3,
					modified = 'A'
				WHERE id_material IS NULL"; // NULL A CONCRETO
		$stmt = $conn->prepare($query);
		$stmt->execute();
		...
		$query =
			"DELETE FROM [auxdw].[servicios_publicos.tbl_luminarias]
				WHERE id = :id";
		$stmt = $conn->prepare($query);
		...
		$query =
			"INSERT INTO [dbo].[tbl_log_updates](username, changed_table)
				VALUES (:username, '[auxdw].[servicios_publicos.tbl_luminarias]')";
		$stmt = $conn->prepare($query);
